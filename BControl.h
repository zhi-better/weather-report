...
public:
	...
	//设置 Edit, Rich Edit 控件是否只读
	// 如果为 true 就是只读，否则为可编辑状态
	void SetReadOnly(bool m_bReadOnly);

	//返回本 Edit 控件的行数
	// 如果是 Edit 类型的控件，返回是行数
	// 如果不是，返回值是 0 
	int GetLineCount();
	...
	//获得和设置 Scroll Bar 控件的那个中间拉的箱子的宽度
	int ScrollBoxWidth();
	void ScrollBoxWidthSet(int iNewValue);

	// ======== Animation 类控件功能 ========
	//打开一段无声媒体
	//并且从头播放，iCycle 表示播放的次数，为 -1 代表循环播放
	bool AnmPlay(tstring lpFileName, int iCycle = -1);
	bool AnmPlay(LPTSTR lpFileName, int iCycle = -1);
	//停止播放媒体，下次播放重新开始
	void AnmStop();

	// ======== Tree View 类控件功能 ========

	//增加一个节点，暂时返回节点的句柄，由自己控制内容
	// lpszItem 节点文本内容
	//iHasChild 表示是否强制显示加号，如果是0表示自动，1表示强制显示加号，-1表示强制不显示
	//nType==0 表示根节点，nType==1 表示儿子节点
	//hNodeTree 为NULL表示默认对当前选中的节点进行操作
	//========================================================================================
	//此操作具有记忆性，如果上次操作的节点并没有被更新，那么以后的增加操作都会在此节点进行
	//好处就是用户不需要每次都在这里输入要插入的节点句柄，大大减少了主调函数的变量定义
	HTREEITEM TreeAddItem(LPTSTR lpszItem, int iHasChild=0, int nType=1, HTREEITEM hNodeTree=NULL);

	//展开或者折叠一个树的内容
	//iAction=true 展开，iAction=false 折叠
	//hNodeTree 表示要操作的节点的句柄
	void TreeExpandSet(int iAction, HTREEITEM hWndNode=NULL);

	//对子节点下面的内容进行排序操作
	bool TreeSortChilden(HTREEITEM hWndNode=NULL);

	//开始编辑一个节点的文本内容
	HWND TreeStartlblEdit(HTREEITEM hWndNode=NULL);

	//停止一个节点文本的编辑
	void TreeEndlblEdit(bool fSave);

	//返回第一个可见的node句柄
	HTREEITEM TreeFirstVisibleNode();

	//返回和设置当前选中的节点的句柄
	HTREEITEM TreeNodeIndex();
	void TreeNodeIndexSet(HTREEITEM hWndNode);

	//获得指定的节点的亲戚节点内容
	//hWndNode 为要开始寻找的节点句柄
	//iType 表示亲戚类型：
	//0 == 祖宗节点
	//1 == 下一个相邻兄弟节点
	//2 == 上一个相邻兄弟节点
	...
	//6 == 下一个可见的相邻兄弟节点
	//7 == 上一个可见的相邻兄弟节点
	HTREEITEM TreeNodeRelatives(int iType, HTREEITEM hWndNode=NULL);

	//获得指定的位置的节点的句柄，感觉这应该是这个的核心代码了吧，哈哈哈哈
	//查找方式：iGroup 表示存储查找的内容在每一级的位置
	//iLevel 表示数组内部的内容的最大级数，从1开始
	//=============== 使用样例：==================
	//获得树状图第一个分组的下面的第二个分组下面的第三个节点的句柄
	// int iGroup[]={1,2,3};
	// form1.Control(...).GetTreeNodeGet(&iGroup, 3);
	HTREEITEM TreeGetByGroup(int* iGroup, int iLevel);

	//删除一个节点内容
	//hWndNode 如果是NULL，那么全部删除
	void TreeDeleteItem(HTREEITEM hWndNode=NULL);

	//获得和设置一个节点的文本内容
	LPTSTR TreeNodeText(HTREEITEM hNodeTree=NULL);
	bool TreeNodeTextSet(LPTSTR sText, HTREEITEM hWndNode=NULL);

	//获得和设置树状控件的文本颜色
	COLORREF TreeNodeColor();
	void TreeNodeColorSet(COLORREF cColor);  

	//为某一个节点创建一个下拉的图片内容
	void TreeNodeCreateDragImage(HTREEITEM hWndNode=NULL);

	//设置和返回一个节点是否显示加号，此处必然是确定设置
	//fHasChild == 1 强制显示加号
	//fHasChild == 0 表示自动设置
	//fHasChild == -1 表示强制不显示加号
	bool TreeNodeHasChildSet(int iHasChild, HTREEITEM hWndNode=NULL);
	int TreeNodeHasChild(HTREEITEM hWndNode=NULL);

	//================ tree控件风格的设置 ==================

	//根节点是否拥有一致的风格
	void TreeStyleATRoot(bool fAtRoot=true);

	//是否要显示线提示信息
	void TreeStyleHasLines(bool fHasLines=true);

	//是否在失去焦点时候也会高亮显示选中的内容
	void TreeStyleHideSelection(bool fHideSelection=true);
...
private:
	...
	TOOLINFO m_tTooltip;			//Tooltip 控件的信息的结构体，在构造函数中自动清零
									//如果添加tooltip文本具体设置
	HWND m_hToolTipWind;		//记录控件绑定的 Tooltip 的控件的句柄
};