...
STWndProp * CBWndBase::PropertyMemCtrl( HWND hWndCtrl, bool bCreate/*=false*/ )
{
	...
	if (bCreate && pProp==NULL)
	{
		...
		if (! ms_hashCtrls.ItemLong2Set((MY_LONG)hWndCtrl, (MY_LONG)pProp, false))
...
COLORREF CBWndBase::BackColor()
{
	...
	if (-1 == m_pPro->backColor)
	{
		...
		HBRUSH hBrush;
		LOGBRUSH lb;
		hBrush=(HBRUSH)GetClassLong(m_hWnd, MYGCL_HBRBACKGROUND);
		if (hBrush) 
		{
...
void CBWndBase::BackColorSet( COLORREF color )
{
	...
	else if (IsClassName( TEXT("RichEdit20W") ))
	{
		//富文本控件，不进行下面的操作，直接返回
		SendMessage(m_hWnd,EM_SETBKGNDCOLOR, 0,(LPARAM)color);
		return;
	}
...
void CBWndBase::TextBackColorSet( COLORREF color )
{
	if (IsClassName( TEXT("RichEdit20W")) )
	{
		//富文本控件，不进行下面的操作，直接返回
		//如果没有设置内容，自动设置选中的内容
		SetFocus();		//设置输入焦点
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_BACKCOLOR;
		lpFormat.crBackColor = color;
		lpFormat.dwEffects &= ~CFE_AUTOBACKCOLOR;	//一定要加，否则直接失败
		SendMessage(m_hWnd,EM_SETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
	}
}
...
COLORREF CBWndBase::ForeColor()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return lpFormat.crTextColor;
	}
...
void CBWndBase::ForeColorSet( COLORREF color, int selStart/*=0*/, int selLength/*=-1*/ )
{
	if ( IsClassName( TEXT("msctls_progress32") ) ) 
	{
		// ProgressBar：不使用XP风格6.0控件时才有效
	...
	else if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_COLOR;
		lpFormat.crTextColor = color;
		lpFormat.dwEffects &= ~CFE_AUTOCOLOR;	//一定要加，否则直接失败
		SendMessage(m_hWnd,EM_SETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return;
	}
...
void CBWndBase::AutoDetectURL( bool fEnabled )
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		//直接发送可以接收到点击url连接的消息
		SendMessage(m_hWnd, EM_SETEVENTMASK, 0, (DWORD)ENM_LINK);
		//富文本控件，不进行下面的操作，直接返回
		if (fEnabled)
			SendMessage(m_hWnd, EM_AUTOURLDETECT, 1, 0);
		else
			SendMessage(m_hWnd, EM_AUTOURLDETECT, 0, 0);
	}
}

int CBWndBase::GetFirstVisibleIndex()
{
...
{
	//针对Rich Edit 需要特殊的处理方式，此处不进行复杂操作
...
LPTSTR CBWndBase::FontName()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_FACE;
		int i = SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		//然后拷贝内容，返回一个完整的内容
		TCHAR * pStr = new TCHAR [lstrlen(lpFormat.szFaceName) + 1];
		HM.AddPtr(pStr);
		_tcscpy(pStr, lpFormat.szFaceName);
		return pStr;
	}
...
void CBWndBase::FontNameSet( LPCTSTR szFontName )
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_FACE;
		_tcscpy(lpFormat.szFaceName, szFontName);
		SendMessage(m_hWnd,EM_SETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return;
	}
...
float CBWndBase::FontSize()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_SIZE;

		SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return (float)lpFormat.yHeight/20;
	}
...
void CBWndBase::FontSizeSet( float fSize )
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_SIZE;
		lpFormat.yHeight = (long)(fSize*20);	//暂时先用这个字体大小的标准吧
		SendMessage(m_hWnd,EM_SETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return;
	}
...
bool CBWndBase::FontBold()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_BOLD;
		SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return (lpFormat.dwEffects&CFE_BOLD)?true:false;
	}
...
bool CBWndBase::FontUnderline()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_UNDERLINETYPE;
		SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return (lpFormat.dwEffects&CFU_UNDERLINENONE)?true:false;
	}
...
void CBWndBase::FontUnderlineSet( bool value, BYTE iType/*=0*/ )
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		//富文本控件，不进行下面的操作，直接返回
...
bool CBWndBase::FontItalic()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_ITALIC;
		SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return (lpFormat.dwEffects&CFE_ITALIC)?true:false;
	}
...
void CBWndBase::FontItalicSet( bool value )
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_ITALIC;
		lpFormat.dwEffects = value ? CFE_ITALIC:~CFE_ITALIC;
		SendMessage(m_hWnd,EM_SETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return;
	}
...
bool CBWndBase::FontShadow()
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		...
		CHARFORMAT2 lpFormat;
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_SHADOW;
		SendMessage(m_hWnd,EM_GETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return (lpFormat.dwEffects&CFE_SHADOW)?true:false;
	}
	else
		return false;
}
...
void CBWndBase::FontShadowSet( bool fShadow )
{
	if (IsClassName( TEXT("RichEdit20W") ))
	{
		//富文本控件，不进行下面的操作，直接返回
		...
		lpFormat.cbSize = sizeof(CHARFORMAT2);
		lpFormat.dwMask = CFM_SHADOW;
		lpFormat.dwEffects = fShadow?CFE_SHADOW:~CFE_SHADOW;	//暂时先用这个字体大小的标准吧
		SendMessage(m_hWnd,EM_SETCHARFORMAT, SCF_SELECTION,(LPARAM)&lpFormat);
		return;
	}
}
...
bool CBWndBase::ToolTipAdd( LPCTSTR sToolTipText, bool f_Balloon/*=true*/, int i_BalloonDirec/*=1*/ )
{
	//创建一个 Tooltip 窗口
	//如果 i_BalloonDirec 不合法，返回失败
	if(i_BalloonDirec !=1 && i_BalloonDirec !=2 && i_BalloonDirec !=4)	return false;
	
	m_pPro = PropMem(true);
	if (m_pPro == NULL)   return false;			// 如果控件创建失败，直接返回失败
	if (m_pPro->hWndTooltip)	return false;	//如果当前已经创建过，返回失败
	
	HWND hParent = GetParent(m_hWnd);		//获得父窗口的句柄
	//设置窗口额提示文本
	HWND m_hToolTipWind=CreateWindowEx(NULL,
		TOOLTIPS_CLASS,
		NULL,
		WS_POPUP | TTS_ALWAYSTIP | (f_Balloon ? TTS_BALLOON : 0),
		CW_USEDEFAULT, 
		CW_USEDEFAULT,
		CW_USEDEFAULT, 
		CW_USEDEFAULT,
	...
	if(!m_hToolTipWind)	
		return false;
	m_pPro->hWndTooltip = m_hToolTipWind;

	TOOLINFO m_tTooltip;
	m_tTooltip.cbSize = sizeof(TOOLINFO); 
	m_tTooltip.hwnd = hParent; 
	//此处，TTF_IDISHWND 代表下面的 uId 为控件的句柄
	// TTF_SUBCLASS 表示，此 tooltip 仅仅会在鼠标移动显示，如果其他复杂功能，不能设置此参数
	m_tTooltip.uFlags = TTF_SUBCLASS | TTF_IDISHWND |i_BalloonDirec; 
	m_tTooltip.uId = (UINT_PTR)m_hWnd; 
	m_tTooltip.lpszText = (LPTSTR)sToolTipText; 
	m_pPro->iTooltipInfo = m_tTooltip;
	SendMessage(m_pPro->hWndTooltip, TTM_ADDTOOL, 0, (LPARAM)&(m_pPro->iTooltipInfo));
	ToolTipWidthSet();

	return true;
}

void CBWndBase::ToolTipTextSet( LPTSTR szText )
{
	m_pPro = PropMem(true);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败

	//直接更改原有的内容
	m_pPro->iTooltipInfo.lpszText = szText;
	SendMessage(m_pPro->hWndTooltip, TTM_UPDATETIPTEXT, 0, (LPARAM)&(m_pPro->iTooltipInfo));
}

LPTSTR CBWndBase::ToolTipText()
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return (LPTSTR)TEXT("");			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return (LPTSTR)TEXT("");	//如果当前没有创建过，返回失败
	//返回对应的Tooltip文本
	return m_pPro->iTooltipInfo.lpszText;
}

void CBWndBase::ToolTipBackColorSet( COLORREF c_Color )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
	//设置提示文本的背景色
	SendMessage(m_pPro->hWndTooltip, TTM_SETTIPBKCOLOR, (WPARAM)c_Color, 0);
}

COLORREF CBWndBase::ToolTipBackColor()
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return 0;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return 0;	//如果当前没有创建过，返回失败
	//获得提示文本的背景色
	return SendMessage(m_pPro->hWndTooltip, TTM_GETTIPBKCOLOR, 0, 0);
}

void CBWndBase::ToolTipTextColorSet( COLORREF c_Color )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
	//获得提示文本的背景色
	SendMessage(m_pPro->hWndTooltip, TTM_SETTIPTEXTCOLOR, (WPARAM)c_Color, 0);
}

COLORREF CBWndBase::ToolTipTextColor()
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return 0;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return 0;	//如果当前没有创建过，返回失败
...
void CBWndBase::TooltipEnabledSet( bool fEnabled )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
	//获得提示文本的背景色
	SendMessage(m_pPro->hWndTooltip, TTM_ACTIVATE, (WPARAM)fEnabled, 0);
}

void CBWndBase::ToolTipInitTimeSet( int iInitTime/*=760*/ )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
	SendMessage(m_pPro->hWndTooltip, TTM_SETDELAYTIME, TTDT_INITIAL, (INT)MAKELONG(iInitTime,0));
}

int CBWndBase::ToolTipInitTime()
{ 
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return 0;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return 0;	//如果当前没有创建过，返回失败
	return SendMessage(m_pPro->hWndTooltip, TTM_GETDELAYTIME, TTDT_INITIAL, 0);
}

void CBWndBase::ToolTipShowTimeSet( int iShowTime/*=7600*/ )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
	SendMessage(m_pPro->hWndTooltip, TTM_SETDELAYTIME, TTDT_AUTOPOP, (INT)MAKELONG(iShowTime,0));
}

int CBWndBase::ToolTipShowTime()
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return 0;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return 0;	//如果当前没有创建过，返回失败
	return SendMessage(m_pPro->hWndTooltip, TTM_GETDELAYTIME, TTDT_AUTOPOP, 0);
}

// void CBWndBase::ToolTipIsBalloonSet( bool fBalloon )
// {
// 	//代码不成熟，待日后重新更改
// 	m_pPro = PropMem(false);
// 	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
// 	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
// 	LONG istyle = GetWindowLong(m_pPro->hWndTooltip, GWL_EXSTYLE);
// 
// 	if (fBalloon)
...
// 		istyle = istyle & ~TTS_BALLOON;
// 
// 	SetWindowLong(m_pPro->hWndTooltip, GWL_EXSTYLE, (LONG)istyle);
// }
// 
// bool CBWndBase::ToolTipIsBalloon()
// {
// 	//代码不成熟，待日后重新更改
// 	m_pPro = PropMem(false);
// 	if (m_pPro == NULL)   return false;			// 如果控件创建失败，直接返回失败
// 	if (!(m_pPro->hWndTooltip))	return false;	//如果当前没有创建过，返回失败
// 
// 	LONG istyle = GetWindowLong(m_pPro->hWndTooltip, GWL_EXSTYLE);
// 
// 	return (bool)(int)(istyle & TTS_BALLOON);
// }

void CBWndBase::ToolTipAlignSet( int iAlign )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;						// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;					//如果当前没有创建过，返回失败
	if(iAlign !=1 && iAlign !=2 && iAlign !=4)	return;	//如果参数不合法，直接返回

	UINT iFlag = m_pPro->iTooltipInfo.uFlags;
	//先删除原来的所有的配置信息
	iFlag = iFlag & ~TTF_CENTERTIP;
	iFlag = iFlag & ~TTF_RTLREADING;
	//设定将要设置的标志变量
	iFlag = iFlag | iAlign;
	m_pPro->iTooltipInfo.uFlags = iFlag;
	SendMessage(m_pPro->hWndTooltip, TTM_SETTOOLINFO, 0, (LPARAM)&(m_pPro->iTooltipInfo));
	
}

int CBWndBase::ToolTipAlign()
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return 0;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return 0;	//如果当前没有创建过，返回失败
	...
	UINT iFlag = m_pPro->iTooltipInfo.uFlags;
	iFlag = iFlag & ~TTF_SUBCLASS;
	if (iFlag & TTF_IDISHWND)
		return 1;
	else
		return (int)iFlag;
}

void CBWndBase::ToolTipWidthSet( int iWidth/*=300*/ )
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return;	//如果当前没有创建过，返回失败
	...
	//设置最大宽度，过于宽会导致看起来不舒服
	SendMessage(m_pPro->hWndTooltip, TTM_SETMAXTIPWIDTH, 0, (LPARAM)iWidth);	
}

int CBWndBase::ToolTipWidth()
{
	m_pPro = PropMem(false);
	if (m_pPro == NULL)   return 0;			// 如果控件创建失败，直接返回失败
	if (!(m_pPro->hWndTooltip))	return 0;	//如果当前没有创建过，返回失败
	
	return SendMessage(m_pPro->hWndTooltip, TTM_GETMAXTIPWIDTH, 0, 0);	
}
