...
// CBForm 类的友元函数
static INT_PTR WINAPI CBForm_DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	// =======================================================================
	...
	case WM_INITDIALOG:
		...
		if (lParam)
		{
			...
			if (CBForm::ms_hashWnd.IsKeyExist((MY_LONG)hWnd))
				CBForm::ms_hashWnd.Remove((MY_LONG)hWnd, false);

			// 向哈希表中添加新项（Key=hwnd，Data=对象地址，
			//   ItemLong=加速键句柄，ItemLong2=受模态对话框影响的 Enabled 状态）
			CBForm::ms_hashWnd.Add((MY_LONG)lParam, (MY_LONG)hWnd, 0, 0);
	...
	default:
		...
		if (CBForm::ms_hashWnd.IsKeyExist((MY_LONG)hWnd))
		{
			pForm = (CBForm*)CBForm::ms_hashWnd.Item((MY_LONG)hWnd, false);
			if (pForm)
			{
...
static int CALLBACK CBForm_ControlProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (CBWndBase::ms_hashCtrls.IsKeyExist((MY_LONG)hWnd))
	{
		// ItemLong = 所位于窗体的 hWnd，可以此为 key 到 ms_hashWnd 中获得窗体
		//   CBForm 对象的地址
		MY_LONG lHWndForm = CBWndBase::ms_hashCtrls.ItemLong((MY_LONG)hWnd, false);
	...
	return CallWindowProc((WNDPROC)(CBWndBase::ms_hashCtrls.Item((MY_LONG)hWnd, false)),
...
static BOOL CALLBACK EnumChildProcSubClass(HWND hWnd, LPARAM lParam)
{
	if (lParam)
	{
		...
		if (!CBWndBase::ms_hashCtrls.IsKeyExist((MY_LONG)hWnd))
		{
			...
			CBWndBase::ms_hashCtrls.Add((MY_LONG)GetWindowLong(hWnd, MYGWL_WNDPROC), (MY_LONG)hWnd,
				(MY_LONG)lParam, 0, 0, 0, 0, false);
			SetWindowLong(hWnd, MYGWL_WNDPROC, (MY_LONG)CBForm_ControlProc);
		}
		...
		if (idCtrl && !CBWndBase::ms_hashCtrlResIDs.IsKeyExist(idCtrl))
			CBWndBase::ms_hashCtrlResIDs.Add((MY_LONG)hWnd, idCtrl,
				(MY_LONG)GetParent(hWnd), (MY_LONG)lParam, 0, 0, 0.0, false);
	...
	else  // if (lParam) else： (lParam == 0)
	{
		...
		if (CBWndBase::ms_hashCtrls.IsKeyExist((MY_LONG)hWnd))
		{
			...
			SetWindowLong(hWnd, MYGWL_WNDPROC,
				CBWndBase::ms_hashCtrls.Item((MY_LONG)hWnd, false));

			// 删除 ms_hashCtrls 和 ms_hashCtrlResIDs 中的对应项目
			CBWndBase::ms_hashCtrls.Remove((MY_LONG)hWnd, false);
			CBWndBase::ms_hashCtrlResIDs.Remove((MY_LONG)GetDlgCtrlID(hWnd), false);
...
CBForm::CBForm(unsigned short int idResDialog /*=0*/) :CBWndBase(NULL, &m_WndProp)
{
	...
	//Tooltip 的资源清理工作
	if (m_pPro->hWndTooltip)	DestroyWindow(m_pPro->hWndTooltip);
	m_pPro->hWndTooltip = NULL;
	memset(&(m_pPro->iTooltipInfo), 0, sizeof(TOOLINFO));
...
{
	if (m_hWnd == NULL) Load(); // 如果窗口还未加载，现在加载
	return (HACCEL)ms_hashWnd.ItemLong((MY_LONG)m_hWnd, false);
}
...
void CBForm::hAccelSet(HACCEL hAccelNew)
{
	...
	ms_hashWnd.ItemLongSet((MY_LONG)m_hWnd, (MY_LONG)hAccelNew, false);	// 记录到 ms_hashWnd 的 ItemLong
...
bool CBForm::Show(int modal/*=0*/, HWND hwndOwner/*=NULL*/)
{
	...
	else
	{
		...
		if (hwndOwner != 0 && hwndOwner != GetWindow(m_hWnd, GW_OWNER))
			SetWindowLong(m_hWnd, -8, (MY_LONG)hwndOwner);
	}
	...
	else
	{
		...
		if (ms_iModalLevel > 0)
		{
			...
			ms_hashWnd.ItemLong2Set((MY_LONG)m_hWnd, ms_iModalLevel, false);
...
HWND CBForm::Load(HWND hwndOwner/*=NULL*/, bool fRaiseErrIfFail/*=true*/)
{
	...
	hwnd = CreateDialogParam(pApp->hInstance, MAKEINTRESOURCE(mResDlgID), hwndOwner,
		CBForm_DlgProc, (MY_LONG)this);	// 立即返回，返回窗口句柄；在窗口过程处理 WM_INITDIALOG 时会设置 m_hWnd
...
CBToolBar* CBForm::Toolbar(unsigned int idCtrlToolBar)
{
	if (m_hWnd == NULL) Load();		//如果窗口还未加载，现在加载

	m_ToolBar.SetHwndFromID(idCtrlToolBar);
...
{
	EventAddPrivate(idResControl, (long)eventType, (MY_LONG)ptrFunHandler, 0);
}
...
{
	EventAddPrivate(idResControl, (long)eventType, (MY_LONG)ptrFunHandler, 1);
}
...
{
	EventAddPrivate(idResControl, (long)eventType, (MY_LONG)ptrFunHandler, 2);
}
	...
	ONEventIII ptrFunHandler)
{
	...
	if (eventType == eMenu_Click)
	{
		...
		m_hashEventNotify.Add((MY_LONG)ptrFunHandler, c_CBMenuClickEventKey, 3, 0, 0, 0, 0, false);
	}
	else
	{
		// ============ 窗体或控件的事件 ============
		EventAddPrivate(idResControl, (long)eventType, (MY_LONG)ptrFunHandler, 3);
	}
...
{
	EventAddPrivate(idResControl, (long)eventType, (MY_LONG)ptrFunHandler, 4);
}

// 设置一个窗体的或子窗口控件的事件（多个重载版本的共用函数）
void CBForm::EventAddPrivate(unsigned short int idResControl, long eventType, MY_LONG ptrFunHandler, int nParamsIntCount)
{
	//ptrFunHandler += 248198;
...
void CBForm::ConstructMenuIDItems(HMENU hMenuParent, bool bIsTop/*=false*/)
{
	...
	for (i = 0; i <= iCount - 1; i++)
	{
		...
		else
		{
			...
			m_hashMenuIDs.Add((MY_LONG)hMenuParent, idItem, 0, 0, 0, 0, 0, false);
		}
...
void CBForm::ClearResource()
{
	...
	if (m_WndProp.hWndTooltip != NULL)
	{
		DestroyWindow(m_WndProp.hWndTooltip);
		m_WndProp.hWndTooltip = NULL;
	}
	memset(&(m_WndProp.iTooltipInfo), 0, sizeof(TOOLINFO));
	...
	if (m_hWnd)
	{
		...
		// 从哈希表中删除该对话框的记录
		ms_hashWnd.Remove((MY_LONG)m_hWnd, false);
...
long CBForm::EventsGenerator(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
	{
		...
		if (m_hashEventNotify.IsKeyExist(key))
		{
			...
			case 4:		// 4 个 int 型参数
				...
				if (ptrFunc4)	// 加 if 容错判断
				{
					//======================= 工具栏消息处理 =========================
					//由于工具栏的消息处理比较奇葩，所以此处另行处置
					switch (((LPNMHDR)lParam)->code)
					{
					case NM_RCLICK:	case NM_CLICK:	case NM_RDBLCLK:
						(*ptrFunc4)((int)((LPNMMOUSE)lParam)->dwItemSpec, (int)((LPNMMOUSE)lParam)->dwItemData,
							(int)((LPNMMOUSE)lParam)->pt.x, (int)((LPNMMOUSE)lParam)->pt.y);
						break;
					case TBN_HOTITEMCHANGE:
						(*ptrFunc4)(0, (int)((LPNMTBHOTITEM)lParam)->dwFlags,
							(int)((LPNMTBHOTITEM)lParam)->idNew, (int)((LPNMTBHOTITEM)lParam)->idOld);
						break;
					default:
						//默认的消息处理函数
						(*ptrFunc4)((int)LOWORD(wParam), (int)HIWORD(wParam),
		...
		}	// end of if (m_hashEventNotify.IsKeyExist(key))
	...
	case WM_ACTIVATE:
		...
		{
			ms_hAccelCurrUsed = (HACCEL)ms_hashWnd.ItemLong((MY_LONG)m_hWnd, false);
		}
		...
		if (uMsg == 0x200)				// 鼠标移动
		{
			...
			if (m_WndProp.iTitleBarBehav || m_iBorderAutoResizable)
			{
				...
				else
				{
					...
					{
						posi = 0; m_WndProp.cursorIdx = m_WndProp.cursorIdx2;
					}	// 鼠标位于窗体内部区域（可以移动窗体位置）
				}
	...
	case WM_USER_NOTIFYICON:
		...
		return 0;   // 不再走 switch 外的生成事件的默认处理；如 break; 则走那个默认处理
	case WM_HOTKEY:
		if (m_hashEventMessage.IsKeyExist((long)WM_HOTKEY))
		{
			ONEventIII ptrFuncForm3 = NULL;

			ptrFuncForm3 =
				(ONEventIII)m_hashEventMessage.Item((long)WM_HOTKEY, false);
			if (ptrFuncForm3)													// 加 if 容错判断
			{
				int idHotKey = (int)wParam;
				UINT shift = (UINT)LOWORD(lParam);
				UINT keyCode = (UINT)HIWORD(lParam);

				// 生成事件
				(*ptrFuncForm3)(idHotKey, (int)shift, (int)keyCode);
			}
		}
		return 0;   // 不再走 switch 外的生成事件的默认处理；如 break; 则走那个默认处理
	case NETWORK_EVENT:
		if (m_hashEventMessage.IsKeyExist((long)NETWORK_EVENT))
		{
			ONEventIII ptrFuncForm3 = NULL;

			ptrFuncForm3 =
				(ONEventIII)m_hashEventMessage.Item((long)NETWORK_EVENT, false);
			if (ptrFuncForm3)													// 加 if 容错判断
			{
				int iSocket = (SOCKET)wParam;
				//调用API函数,得到网络事件类型
				int iEvent = WSAGETSELECTEVENT(lParam);
				int iError = WSAGETSELECTERROR(lParam);   // ie, HIWORD

				// 生成事件
				(*ptrFuncForm3)(iSocket, iEvent, iError);
			}
		}
		return 0;   // 不再走 switch 外的生成事件的默认处理；如 break; 则走那个默认处理
		// 	case :
	...
	if (m_hashEventMessage.IsKeyExist((long)uMsg))
	{
		...
		case 1:		// 1 个 int 型参数
			...
			if (ptrFuncForm1) (*ptrFuncForm1)((MY_INT)wParam);					// 加 if 容错判断
			break;
		case 2:		// 2 个 int 型参数
			ptrFuncForm2 =
				(ONEventII)m_hashEventMessage.Item((long)(uMsg), false);
			if (ptrFuncForm2) (*ptrFuncForm2)((MY_INT)wParam, (MY_INT)lParam);	// 加 if 容错判断
			break;
		case 3:		// 3 个 int 型参数
			ptrFuncForm3 =
				(ONEventIII)m_hashEventMessage.Item((long)(uMsg), false);
			if (ptrFuncForm3) (*ptrFuncForm3)((MY_INT)LOWORD(wParam), (MY_INT)HIWORD(wParam),
				(MY_INT)lParam);												// 加 if 容错判断
			break;
		case 4:		// 4 个 int 型参数
			ptrFuncForm4 =
				(ONEventIIII)m_hashEventMessage.Item((long)(uMsg), false);
			if (ptrFuncForm4) (*ptrFuncForm4)((MY_INT)LOWORD(wParam), (MY_INT)HIWORD(wParam),
				(MY_INT)LOWORD(lParam), (MY_INT)HIWORD(lParam));					// 加 if 容错判断
			break;
		}
	}	// if (m_hashEventMessage.IsKeyExist((long)uMsg))
...
long CBForm::EventsGeneratorCtrl(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		if (ms_hashCtrls.IsKeyExist((MY_LONG)hWnd))		// 如果该控件被本程序管理
		{
			CBControl ctrl;
			if (ctrl.SetResID(idCtrl))		// 且，如果成功按照 idCtrl 获得了控件(idCtrl 可能无效导致失败)
			{
				...
				else	// else - if ( fPicStatic )
				{
					...
					CallWindowProc((WNDPROC)(CBWndBase::ms_hashCtrls.Item((MY_LONG)hWnd, false)),
...
{
	// 若当前无托盘，返回 ""（先设置 m_NIData.szInfo 为 ""，后返回）
	if (!m_NIData.cbSize) *m_NIData.szInfo = 0;
...
int CBForm::SysHotkeySet(int keyCode, int shift/*=0*/, bool bShowMsg/*=false*/, int idHotkey/*=-1*/)
{
	// 如果此时是第一次设置，先给那个标志的 bool 数组初始化全部为 false 
	int i = 0;
	if (idSysHotkey == -1)
	{
		for (i = 0; i < MaxSysHotKey; i++)
		{
			m_fHotkeyState[i] = false;
		}
	}

	//如果为负号，返回失败
	if (keyCode < 0 || shift < 0 || idHotkey < -1 || idHotkey >MaxSysHotKey)
		return -1;

	// 如果用户是默认的选项，那么用通用模块的 id
	if (idHotkey == -1)
	{
		idSysHotkey++;
		// 如果超出 最大的 ID ，那么首先查找看看有没有中间被取消的热键 id
		//如果有用这个，否则返回失败
		if (idSysHotkey > MaxSysHotKey)
		{
			for (i = 0; i <= MaxSysHotKey; i++)
			{
				if (!m_fHotkeyState[i])	break;
			}

			//如果有那就用回收的，没有返回失败
			//不过我就不信能有人这么狠用这么多热键
			if (i > MaxSysHotKey)	return -1;
			else	idHotkey = i;
		}
		else
			idHotkey = idSysHotkey;
	}
	else
	{
		//如果用户给的这个 id 过大，这里要及时的更新这个内部静态成员的数值，
		// 防止最后释放资源不全
		if (idHotkey > idSysHotkey)
			idSysHotkey = idHotkey;

		if (m_fHotkeyState[idSysHotkey])
		{
			//如果热键有效，先删除原有的热键，然后设置
			SysHotkeyRemove(idHotkey);
		}
	}

	BOOL ret = RegisterHotKey(m_hWnd, idHotkey, shift, keyCode);

	// 是否显示成功或者失败的设置的信息
	if (ret == 0)
	{
		if (bShowMsg)
			MsgBox(TEXT("热键定义失败，请重新指定一个其他的热键。"));

		// 为了不浪费 id ，这里就让这个静态成员减减
		idSysHotkey--;
		return -1;
	}
	else
	{
		...
		{
			tstring sMsg = TEXT("");
			sMsg = StrAppend(sMsg.c_str(), TEXT("系统热键设置成功，热键组合为："),
				(((shift & 1) == 1) ? TEXT("alt + ") : TEXT("")),
				(((shift & 2) == 2) ? TEXT("ctrl + ") : TEXT("")),
				(((shift & 4) == 4) ? TEXT("shift + ") : TEXT("")),
				(((shift & 2) == 8) ? TEXT("win + ") : TEXT("")),
				Str(keyCode));
			sMsg = sMsg + TEXT("\r\n热键的 id 是：") + Str(idHotkey);
			MsgBox(sMsg);
		}
		// 如果成功就保存那个状态为 true 
		m_fHotkeyState[idSysHotkey] = true;
		return idSysHotkey;
	}

}

bool CBForm::SysHotkeyRemove(int idHotkey /*= -1*/, bool fRemoveAll /*= false*/)
{
	...
	{
		//如果 id 的下标小于-1，返回失败
		if (idHotkey < -1)	return false;

		UnregisterHotKey(m_hWnd, idHotkey);
		m_fHotkeyState[idHotkey] = false;
	}
...
//静态成员赋初值
// 初值为 -1 是为了刚好和数组一致
int CBForm::idSysHotkey = -1;

//////////////////////////////////////////////////////////////////////////
// 为了更加简单的使用 char 和 TCHAR
// 此处提供了转换的两个函数
//////////////////////////////////////////////////////////////////////////
...
tstring RGBtoText(COLORREF cColor)
{
	////将 RGB 转换为字符串数据，便于使用
	BYTE Red = GetRValue(cColor); ///得到红颜色
	BYTE Green = GetGValue(cColor); ///得到绿颜色
	BYTE Blue = GetBValue(cColor); ///得到兰颜色

	return StrPrintf(TEXT("RGB(%d,%d,%d)"), Red, Green, Blue);
}

void CreateKeybdEvent(int idKey, int iType /*= 0*/, int iDiffTime /*= 10*/)
{
	switch (iType)
	{
	case 0:
		keybd_event(idKey, 0, KEYEVENTF_EXTENDEDKEY | 0, 0);
		Sleep(iDiffTime);
		keybd_event(idKey, 0, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP, 0);
		Sleep(iDiffTime);
		break;
	case -1:
		keybd_event(idKey, 0, KEYEVENTF_EXTENDEDKEY | 0, 0);
...
tstring KeycodeToStr(int iKeycode)
{
	tstring sKeyName = TEXT("");
	switch (iKeycode)
	{
	case VK_ESCAPE:
		sKeyName = TEXT("Esc");
