...
typedef struct _WndProp			 
{
	...
	HWND hWndTooltip;			//如果关联了一个 Tooltip 的提示窗口，提示窗口的句柄用此变量保存
	TOOLINFO iTooltipInfo;		//保存的是当前的 Tooltip 的具体内容信息
...
public:
	...
	//返回和设置rich edit的文本的背景色
	COLORREF TextBackColor();
	void TextBackColorSet(COLORREF color);

	// 返回和设置 窗体或控件 的前景色（如用于输出文本的文本颜色）
	// 也可用于设置进度条控件前景色（“条”的颜色）（不使用XP风格6.0控件时才有效）
	COLORREF ForeColor();
	void ForeColorSet(EColorType color);
	void ForeColorSet(COLORREF color, int selStart=0, int selLength=-1);

	//设置是否自动侦测URL文本的功能
	void AutoDetectURL(bool fEnabled);

	//获得当前可见的第一行的行号，仅仅针对Rich Edit控件
	//编号从0开始算起吧，如果失败返回-1
	...
	// 返回和设置 窗口字体是否加下划线
	// iType 表示的是下下划线的类型，仅仅针对Rich Edit 有用
	// iType = 0 表示没有下划线
	// iType = 1 表示单条下划线
	// iType = 2 表示单词下面有下划线
	// iType = 3 表示双下划线
	// iType = 4 表示点下划线
	bool FontUnderline();
	void FontUnderlineSet(bool value, BYTE iType=0);
	...
	//返回和设置字体是否具有阴影效果，针对Rich Edit 控件
	//效果似乎不太明显，感觉会重一点吧
	bool FontShadow();
	void FontShadowSet(bool fShadow);
	...
	//为句柄为 hWnd 的控件增加一个提示文本, tooltip
	//idCtrl 表示要增加提示文本的控件
	//sToolTipText 表示Tooltip将要显示的提示文本
	//f_Balloon 表示显示的提示文本是气球还是矩形
	//f_BalloonDirec 表示显示的提示框的相对位置，1 = 右侧 2 = 中间 4 = 左侧
	bool ToolTipAdd(LPCTSTR sToolTipText, bool f_Balloon=false, int i_BalloonDirec=1);

	//获得和设置一个控件的提示文本内容
	void ToolTipTextSet(LPTSTR szText);
	LPTSTR ToolTipText();

	//获得和设置一个 Tooltip 的背景颜色
	void ToolTipBackColorSet(COLORREF c_Color);
	COLORREF ToolTipBackColor();

	//获得和设置一个 Tooltip 的字体颜色
	void ToolTipTextColorSet(COLORREF c_Color);
	COLORREF ToolTipTextColor();

	//设置是否使能显示状态
	void TooltipEnabledSet(bool fEnabled);

	//获得和设置一个 Tooltip 的显示之前延迟的时间长度的时间长度
	//iInitTime 单位：ms
	void ToolTipInitTimeSet(int iInitTime=760);
	//返回当前的初始化的时间长度（显示之前延迟的时间）
	//单位：ms
	int ToolTipInitTime();

	//获得和设置一个 Tooltip 的Tooltip显示后持续的时间长度
	//iShowTime 单位：ms
	void ToolTipShowTimeSet(int iShowTime=7600);
	//返回当前的Tooltip的显示后的持续显示的时间
	//单位：ms
	int ToolTipShowTime();
	
	//返回和设置窗口是否为气泡窗口
	void ToolTipIsBalloonSet(bool fBalloon);
	bool ToolTipIsBalloon();

	//设置 Tooltip 的相对于鼠标的位置
	//1 = 左侧 2 = 中间 4 = 右侧
	//如果是 2，那么显示的Tooltip窗口位置不会随着鼠标的位置改变，显示在控件的中间位置
	void ToolTipAlignSet(int iAlign);
	int ToolTipAlign();

	//获得和设置 tooltip 的最大宽度
	void ToolTipWidthSet(int iWidth=300);
	int ToolTipWidth();
