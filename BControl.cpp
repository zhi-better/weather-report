...
int CBControl::SelStart(int *pLineIdx/*=NULL*/)
{
	...
	else if (_tcsstr(ClassName(), TEXT("RICHEDIT")) || _tcsstr(ClassName(), TEXT("RichEdit20W")))	// 以 Rich Edit 控件的方式操作
	{
		CHARRANGE cr;
...
LPTSTR CBControl::SetText()
{
	int iLength = SelLength();
	TCHAR * pText=new TCHAR[iLength+1];
	HM.AddPtr(pText);

	SendMessage(m_hWnd,  EM_GETSELTEXT, 0, (LPARAM)pText);
	return pText;
}
...
{
	if (_tcsstr(ClassName(), TEXT("RICHEDIT"))==0 || _tcsstr(ClassName(), TEXT("RichEdit20W"))==0) return -1;	// 不支持控件
...
void CBControl::EditSetApperance( unsigned short idPicBorder, unsigned short idBMPBorder, int x, int y, int width, int height, LPTSTR szFontName/*=NULL*/, float fFontSize/*=10.5*/, int xOffset/*=4*/, bool bOnlyMoveSize/*=false*/ )
{
	...
	if (! szFontName) szFontName=(LPTSTR)TEXT("宋体");  // 默认为“宋体”
...
int CBControl::Value()
{
	...
	else if (IsClassName(TEXT("msctls_trackbar32")))
	{
		// Trackbar Bar(Slider)
		return SendMessage(m_hWnd, TBM_GETPOS, 0, 0);
	}
	else if (IsClassName(TEXT("ScrollBar")))
	{
		// Scroll Bar
...
void CBControl::ValueSet( int iNewValue )
{
	...
	{
		// Trackbar Bar(Slider)
		SendMessage(m_hWnd, TBM_SETPOS, 1, (LPARAM)iNewValue);
	}
	else if (IsClassName(TEXT("ScrollBar")))
	{
		// Scroll Bar
		SendMessage(m_hWnd, SBM_SETPOS, iNewValue, (LPARAM)TRUE);
	}
}
...
int CBControl::Max()
{
	...
	else if (IsClassName(TEXT("ScrollBar")))
	{
		...
		int iValueMax = 0;
		int iValueMin = 0;
		SendMessage(m_hWnd, SBM_GETRANGE, (WPARAM)&iValueMin, (LPARAM)&iValueMax);
		return iValueMax;
	}
...
void CBControl::MaxSet( int iNewValue )
{
	...
	{
		// Trackbar Bar(Slider)
		SendMessage(m_hWnd, TBM_SETRANGEMAX, 1, (LPARAM)iNewValue);
	}
	else if (IsClassName(TEXT("ScrollBar")))
	{
		// Scroll Bar
		SendMessage(m_hWnd, SBM_SETRANGE, (WPARAM)Min(), (LPARAM)iNewValue);
	}
}
...
void CBControl::MinSet( int iNewValue )
{
	...
	{
		// Trackbar Bar(Slider)
		SendMessage(m_hWnd, TBM_SETRANGEMIN, 1, (LPARAM)iNewValue);
	}
	else if (IsClassName(TEXT("ScrollBar")))
	{
		// Scroll Bar
		SendMessage(m_hWnd, SBM_SETRANGE, (WPARAM)iNewValue, (LPARAM)Max());
	}
}
...
void CBControl::SetReadOnly( bool m_bReadOnly )
{
	//先判断是不是 Edit 控件，然后进行相应的操作
	if ( IsClassName(TEXT("Edit")) || IsClassName(TEXT("RichEdit20W")) )
		SendMessage(m_hWnd, EM_SETREADONLY, m_bReadOnly, 0);

}

int CBControl::GetLineCount()
{
	//先判断是不是 Edit 控件，然后进行相应的操作
	//获得当前的 Edit 的文字行数
	//如果不是该类型控件，返回 0 
...
bool CBControl::AnmPlay(LPTSTR lpFileName, int iCycle /*= -1*/)
{
	//打开 lpFileName 的图像文件
	//成功返回 true ，失败返回 false
	if ( IsClassName(TEXT("SysAnimate32")) )
	{
		//如果打开成功，然后发送播放消息
		if(SendMessage(m_hWnd, ACM_OPEN, NULL, (LPARAM)lpFileName))
		{
			SendMessage(m_hWnd, ACM_PLAY, (WPARAM)iCycle, (LPARAM) MAKELONG(0,-1));
			return true;
		}
		else
			return false;
...
bool CBControl::AnmPlay( tstring lpFileName, int iCycle /*= -1*/ )
{
	//重载版本，更加通用
	return AnmPlay((LPTSTR)lpFileName.c_str());
}

void CBControl::AnmStop()
{
	//如果类型正确，然后发送暂停消息
	if ( IsClassName(TEXT("SysAnimate32")) )
		SendMessage(m_hWnd, ACM_PLAY, (WPARAM)-1, (LPARAM) MAKELONG(0,-1));
}

HTREEITEM CBControl::TreeAddItem( LPTSTR lpszItem, int iHasChild /*=0*/, int nType /*=1*/, HTREEITEM hWndNode /*=NULL*/ )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return NULL;

	TVITEM tvi; 
	TVINSERTSTRUCT tvins; 
	static HTREEITEM hWndTemp = NULL;
	...
	if (iHasChild == 1)
	{
		tvi.mask = tvi.mask | TVIF_CHILDREN;
		tvi.cChildren = iHasChild;
	}
	else if (iHasChild == -1)
	{
		tvi.mask = tvi.mask | TVIF_CHILDREN;
	...
	//默认是在根节点插入内容，否则是对应句柄的节点
	if (nType == 0)
		tvins.hParent = TVI_ROOT;	//父亲是最根节点
	else
		tvins.hParent = hWndTemp;
	tvins.item = tvi;								//设置 Item 的这个结构体的内容
	tvins.hInsertAfter = (HTREEITEM)TVI_LAST;		//设置插入的位置，默认都是第一个

	return (HTREEITEM) SendMessage(m_hWnd, TVM_INSERTITEM, 0, (LPARAM)&tvins); //返回创建的句柄	
}

void CBControl::TreeExpandSet( int iAction, HTREEITEM hWndNode/*=NULL*/ )
{
	//先判断是否是树形控件
	if (IsClassName(TEXT("SysTreeView32")))
	{
		//如果没有指定 hWndNode 的值，自动获得当前选中的节点的句柄
		if(hWndNode == NULL)
			hWndNode = TreeNodeIndex();
		SendMessage(m_hWnd,TVM_EXPAND, iAction ? TVE_EXPAND : TVE_COLLAPSE, (LPARAM)hWndNode);
	}
}

bool CBControl::TreeSortChilden( HTREEITEM hWndNode/*=NULL*/ )
{
	//先判断是否是树形控件
	if (IsClassName(TEXT("SysTreeView32")))
	{
		...
		if(hWndNode == NULL)
			hWndNode = TreeNodeIndex();
		SendMessage(m_hWnd,TVM_SORTCHILDREN, 0, (LPARAM)hWndNode);
		return true;
	}
	else
		return false;
...
HWND CBControl::TreeStartlblEdit( HTREEITEM hWndNode/*=NULL*/ )
{
	if (IsClassName(TEXT("SysTreeView32")))
	{
		//如果没有指定 hWndNode 的值，自动获得当前选中的节点的句柄
		if(hWndNode == NULL)
			hWndNode = TreeNodeIndex();
		SetFocus();		//需要先设置焦点
		return (HWND)SendMessage(m_hWnd,TVM_EDITLABEL, 0, (LPARAM)hWndNode);
	}
	else
		return NULL;
}

void CBControl::TreeEndlblEdit( bool fSave )
{
	if (IsClassName(TEXT("SysTreeView32")))
	{
		SendMessage(m_hWnd,TVM_ENDEDITLABELNOW, (BOOL)fSave, 0);
	}
}

HTREEITEM CBControl::TreeFirstVisibleNode()
{
	if (IsClassName(TEXT("SysTreeView32")))
	{
...
LPTSTR CBControl::TreeNodeText( HTREEITEM hWndNode/*=NULL*/ )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return (LPTSTR)TEXT("");
	...
	//获得设置信息的方式，哦耶，知道了
	TVITEM tvi;
	TCHAR * pText=new TCHAR[256];
	HM.AddPtr(pText);
	tvi.mask=TVIF_TEXT;    //要获得文本内容
	tvi.pszText=pText;      //文本要保存的地址内容
	tvi.cchTextMax=256;    //地址最大长度
	tvi.hItem=hWndNode;    //要查找的内容的句柄

	//成功返回文本，失败返回空字符串
	if(SendMessage(m_hWnd, TVM_GETITEM, TVGN_CARET, (LPARAM)&tvi))
		return pText;
	else
		return (LPTSTR)TEXT("");
}

bool CBControl::TreeNodeTextSet( LPTSTR sText, HTREEITEM hWndNode/*=NULL*/ )
{
	//设置一个控件节点的文本内容
	if (IsClassName(TEXT("SysTreeView32")))
	{
		...
		tvi.mask = TVIF_TEXT;
		tvi.pszText = sText;
		return SendMessage(m_hWnd,TVM_SETITEM, 0, (LPARAM)&tvi)==TRUE ? true:false;
	}
	else
		return false;
}

HTREEITEM CBControl::TreeNodeIndex()
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return NULL;
...
void CBControl::TreeNodeIndexSet( HTREEITEM hWndNode )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return;
	...
	//设置内容
	TreeView_SelectItem(m_hWnd, hWndNode);	
	TreeView_EnsureVisible(m_hWnd, hWndNode);
	SetFocus();		//确保显示能够看到
}

HTREEITEM CBControl::TreeNodeRelatives( int iType, HTREEITEM hWndNode/*=NULL*/ )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return NULL;
	...
	//然后返回指定的内容
	return TreeView_GetNextItem(m_hWnd, hWndNode, iType);
}

HTREEITEM CBControl::TreeGetByGroup( int* iGroup, int iLevel )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return NULL;
	
	HTREEITEM hNodeTemp=NULL;
	hNodeTemp = TreeNodeRelatives(0, 0);	//先获得祖宗节点
	//如果获得失败就是空白，如果是空白，直接返回空
	if(hNodeTemp == NULL)
		return NULL;
	int i,j;
	for (j=0;j<iLevel;j++)
	{
		for (i=1;i<iGroup[j];i++)
		{
			//一点点获得兄弟节点的句柄
			hNodeTemp = TreeNodeRelatives(2,hNodeTemp);
			//如果获得失败就是空白，如果是空白，直接返回空
			if(hNodeTemp == NULL)
				return NULL;
		}
		//如果此处已经查完了，就不再进行儿子的查找了，直接返回就行了
		if(j == iLevel-1)	break;

		//然后找到指定的兄弟节点的句柄后，获得指定的孩子节点
		hNodeTemp = TreeNodeRelatives(3,hNodeTemp);
		//如果获得失败就是空白，如果是空白，直接返回空
		if(hNodeTemp == NULL)
			return NULL;
	}
	//如果循环够一遍，那么就返回正确的位置的句柄
	return hNodeTemp;
}

void CBControl::TreeDeleteItem( HTREEITEM hWndNode/*=NULL*/ )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return;
	...
	if (hWndNode == NULL)
		...
		TreeView_DeleteItem(m_hWnd, hWndNode);
}

COLORREF CBControl::TreeNodeColor()
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return -1;
...
void CBControl::TreeNodeColorSet( COLORREF cColor )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return;

	TreeView_SetTextColor(m_hWnd, cColor);
}

void CBControl::TreeNodeCreateDragImage(HTREEITEM hWndNode /*=NULL*/)
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return;
	...
	if(hWndNode == NULL)
		TreeView_CreateDragImage(m_hWnd,TreeNodeIndex());
	else
		TreeView_CreateDragImage(m_hWnd,hWndNode);
}

bool CBControl::TreeNodeHasChildSet( int iHasChild, HTREEITEM hWndNode /*=NULL*/ )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return false;
...
int CBControl::TreeNodeHasChild( HTREEITEM hWndNode/*=NULL*/ )
{
	//如果不是指定的控件，直接返回
	if (!IsClassName(TEXT("SysTreeView32")))
		return 0;
	...
	//获得设置信息的方式，哦耶，知道了
	TVITEM tvi;
	tvi.mask=TVIF_CHILDREN;    //要获得是否显示加号的情况
	tvi.hItem=hWndNode;			//要查找的内容的句柄
...
void CBControl::TreeStyleATRoot( bool fAtRoot/*=true*/ )
{
	if (IsClassName(TEXT("SysTreeView32")))
	{
		if (fAtRoot)
			StyleExSet(TVS_LINESATROOT,1);
		else
			StyleExSet(TVS_LINESATROOT,-1);
	}
}

void CBControl::TreeStyleHasLines( bool fHasLines/*=true*/ )
{
	if (IsClassName(TEXT("SysTreeView32")))
	{
		if (fHasLines)
			StyleExSet(TVS_HASLINES,1);
		else
			StyleExSet(TVS_HASLINES,-1);
	}
}

void CBControl::TreeStyleHideSelection( bool fHideSelection/*=true*/ )
{
	if (IsClassName(TEXT("SysTreeView32")))
	{
		if (fHideSelection)
			StyleExSet(TVS_SHOWSELALWAYS,1);
		else
			StyleExSet(TVS_SHOWSELALWAYS,-1);
	}
}

int CBControl::ScrollBoxWidth()
{
	//如果不是这种控件，自动返回 0
	if (IsClassName(TEXT("ScrollBar")))
	{
		// Scroll Bar
		SCROLLINFO  si;  
		si.cbSize=sizeof(SCROLLINFO);  
		si.fMask=SIF_PAGE;  
		GetScrollInfo(m_hWnd,SB_CTL,&si);//先拿滚动条信息 
		return si.nPage;
	}
...
void CBControl::ScrollBoxWidthSet( int iNewValue )
{
	//设置当前的 Scroll Bar 的宽度，不能超出最大的宽度
	if (IsClassName(TEXT("ScrollBar")))
	{
		// Scroll Bar
		SCROLLINFO  si;  
		si.cbSize=sizeof(SCROLLINFO);  
		si.fMask=SIF_PAGE;  
		si.nPage = min(iNewValue, Max());
		SetScrollInfo(m_hWnd, SB_CTL, &si, true);//先拿滚动条信息 
	}
}
