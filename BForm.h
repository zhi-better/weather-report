...
//数字按键的宏定义，注意此处定义的内容是大键盘的数字按键
//小键盘的数字按键定义形式为 VK_NUMPAD0 注意不能混用
#define VK_0	48
#define VK_1	49
#define VK_2	50
#define VK_3	51
...
#define WM_UARTREAD WM_USER + 121		//自定义的消息，串口接收到了一个字节的内容
#define WM_TTS WM_USER+500				//定义用来语音识别的事件
#define WM_HTTP_DOWNLOAD WM_USER+200	//定义的用于下载文件查看进度的消息
...
enum ECBFormCtrlEventsVoid		
{
	...
	eComboBox_GetFocus = CBN_SETFOCUS | c_CBNotifyEventBase,		//void Combo1_CboGetFocus(int notUsed, int hwndComboBox)
	eComboBox_LostFocus = CBN_KILLFOCUS | c_CBNotifyEventBase,		//void Combo1_CboLostFocus(int notUsed, int hwndComboBox)


	eTreeView_Click = NM_CLICK | c_CBNotifyEventBase,				//void TreeView_Click(); 树状图控件被左键单击产生的消息
	eTreeView_DBClick = NM_DBLCLK | c_CBNotifyEventBase,			//void TreeView_DBClick(); 树状图控件被左键双击产生的消息
	eTreeView_LoseFocus = NM_KILLFOCUS | c_CBNotifyEventBase,		//void TreeView_LoseFocus(); 树状图控件失去焦点产生的消息
	eTreeView_RCLICK = NM_RCLICK  | c_CBNotifyEventBase,			//void TreeView_RClick(); 树状图控件被右键单击产生的消息
	eTreeView_GetFocus = NM_SETFOCUS  | c_CBNotifyEventBase,		//void TreeView_GetFocus(); 树状图控件获得输入焦点产生的消息

	eDateTime_Change = DTN_DATETIMECHANGE | c_CBNotifyEventBase,		//这个是对应的日期时间控件内容改变发送的消息	
...
enum ECBFormCtrlEventsII
{
	...
	eDateTimeChange = DTN_DATETIMECHANGE | c_CBNotifyEventBase,	//void DateTime_DateTimeChange(int notUsed, int pDateTimeInfo);
											//其中通过 ((LPNMDATETIMECHANGE)pDateTimeInfo)->st.时间
											//获得对应的时间内容

	
	eTreeView_EndLabelEdit = TVN_ENDLABELEDIT | c_CBNotifyEventBase,		//void Tree_EndLabelEdit(int notUsed, int pTreeInfo);
										//使用时将第二个参数强制类型转换为 LPNMTVDISPINFO
										//然后获得的 ((LPNMTVDISPINFO)pTreeInfo)->item.pszText; 就是对应的更改后的文本内容
										//需要手动将句柄为((LPNMTVDISPINFO)pTreeInfo)->item.hItem 的节点的文本改变，不会自动改变内容


	eEditRtf_URLClick = EN_LINK | c_CBNotifyEventBase,				//void EditRtf_URLClick(int notUsed, int pEnlink);	
													//仅支持 Rich Edit
													// pEnlink (ENLINK *)类型的变量，使用需要进行类型转换，然后自己判断选中的内容

	eListViewColumnClick = LVN_COLUMNCLICK,	//注意逗号
											//void listView_ColumnClick(int notUsed, int lParam)
											//使用过程中需要把 lParam 强制转换为 LPNMLISTVIEW 类型的指针
	
	eToolBar_KeyDown = NM_KEYDOWN | c_CBNotifyEventBase,	//对应的是toolbar控件获得输入焦点并且按下按键
										//void Form1_ToolBarKeyDown(int notUsed, int lpNmkey);
										//使用时需要用 ((LPNMKEY)lpNmkey)->nVKey 获得键码


	// 以其他消息产生的事件：枚举值 = WM_XXX
	//   此枚举值将作为 m_hashEventNotify 的各项 Key 的低 2 位
	//   （注意不是高2位，高2位为0表示窗体事件，为控件ID(非0)表示控件事件）
	eReadUsart = WM_UARTREAD,				// void Form1_ReadUsart(int iCom, int iData);  
											// iCom 表示的是端口号
											// iData 表示的是接收的一个字节的数据

	eKeyPress = WM_CHAR,					// void Form1OrCtrl1_KeyPress(int keyAscii, int pbCancel);  // 将 *((int *)pbCancel) = 非0值，将取消
										
										//具体用法查阅 MM_MCINOTIFY 消息的内容
										//wFlags 表示的是事件的标识位
										//lDevID 回调函数的设备标识
	eMciNotify = MM_MCINOTIFY,			//void form1_MciNotify(int wFlags, int lDevID);

	eHttpDownload = WM_HTTP_DOWNLOAD	//void Form1_HttpDownload(int hRequest, int pWinInetInfo)
										//其中使用的时候第二个参数需要转换为 (WinInetInfo*)pWinInetInfo指针
...
enum ECBFormCtrlEventsIII
{
	...
	eSysTrayMouseEvent = WM_USER_NOTIFYICON,		// void Form1_SysTrayMouseEvent(int button, int action, int idSysTray)	
												// button=1,2,4 分别代表鼠标左键、右键、中键按键的动作
												// action=0,1,2,3 分别表示鼠标移动、按下、抬起、双击
	eSysHotkey = WM_HOTKEY,				// void Form1_SysHotkey(int idHotKey, int shift, int keyCode);
										// idHotKey 代表的是热键的 ID
										// shift=1,2,4 分别代表 Alt Shift Ctrl 为修饰按键
										// keyCode 代表的是虚拟键码
				
									//void Form1orCtrll_Hscroll(int action, int iPos, int hWnd); 
									// action=0,1 分别表示这个 Scroll Bar 的左右侧（上下侧）的箭头被点击
									//		=2,3 分别表示这个 Scroll Bar 的左右侧（上下侧）的空白位置被点击
									//		=5,8 分别表示这个 Scroll Bar 的中间的 Box 正在被拖动和鼠标已经离开拖动的 Box
									// iPos 表示的是当卡的这个中间的 Box 的位置，如果action !=8，此值无效，为0
	eHScroll = WM_HSCROLL,		// hWnd 表示的是发生事件的这个控件的句柄
									...
									//		=5,8 分别表示这个 Scroll Bar 的中间的 Box 正在被拖动和鼠标已经离开拖动的 Box
									// iPos 表示的是当卡的这个中间的 Box 的位置，如果action !=8，此值无效，为0
	eVScroll = WM_VSCROLL,		// hWnd 表示的是发生事件的这个控件的句柄

									//iSocket 表示的是发出此事件的socket的句柄
									//iEvent 代表的是socket发出的事件类型，对应以FD_...
									//iError 表示的是错误代码
	eNetWorkEvent = NETWORK_EVENT	//void form1_NetWorkEvent(int iSocket, int iEvent, int iError);
...
enum ECBFormCtrlEventsIIII
{
	...
	eToolBar_RClick = NM_RCLICK | c_CBNotifyEventBase,	//对应的是toolbar控件被鼠标右键单击产生的消息
												//void Form1_ToolBarRClick(int iCommand, int iData, int x, int y);

	eToolBar_Click = NM_CLICK | c_CBNotifyEventBase,	//对应的是toolbar控件被鼠标右键单击产生的消息
										//void Form1_ToolBarClick(int iCommand, int iData, int x, int y);
	
//	//内容转换有问题，此消息暂时不做处理
// 	eToolBar_RDBClick = NM_RDBLCLK | c_CBNotifyEventBase,	//对应的是toolbar控件被鼠标右键双击产生的消息
// 										//void Form1_ToolBarRDBClick(int iCommand, int iData, int x, int y);
 	
	eToolBar_ItemChange = TBN_HOTITEMCHANGE | c_CBNotifyEventBase,	//对应的是toolbar控件被鼠标右键单击产生的消息
										//void Form1_ToolBarItemChange(int notUsed, int iFlags, int idNew, int idOld);
	...
	eMouseWheel = WM_MOUSEWHEEL,	// void Form1OrCtrl1_MouseWheel(int shift, int distance, int x, int y);
									//   shift=1,2,4,8,16 分别表示鼠标左键，鼠标右键，shift，ctrl，鼠标中键被按下
									//   distance 表示鼠标的滚轮移动的距离（65416），每次都会发送，可以不关注此参数
									//   x,y 表示鼠标在控件中的坐标（控件坐标系）
	...
	//eHttpDownload = WM_HTTP_DOWNLOAD// void Form1_HttpDownload(int hWndHttp, int iLenAll, int iLenDownload, int iTime);
									// 参数说明：下载的http请求的句柄，下载的总长度，现在已经接收到的消息的长度，网速
...
private:
	...
	// 用于保存设置的系统热键的 ID
	// 静态成员保证ID每次增加1，老师书里说的那个实现回收先不说吧
	static int idSysHotkey;

	// 定义常量，用于控制系统热键 ID 的最大值
	static const int MaxSysHotKey = 0x7fff;
...
public:
	...
	// 公用窗口过程：所有本类对象（窗体）都用此函数作为窗口过程
	friend INT_PTR WINAPI CBForm_DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
public:
	...
	// 返回一个工具栏项（CBToolBar 对象，实际返回绑定具体菜单项后的 m_ToolBar），
	// 之后可对该工具栏进行操作（调用 CBToolBar 类的属性方法）
	CBToolBar* Toolbar(unsigned int idCtrlToolBar);
	...
	// 设置系统热键
	// 如果设置成功，返回的是热键的 id（id默认大于等于0） ,如果设置失败，返回的是 -1
	// keyCode 代表的是设置的热键的键码
	// shift 代表的是设置的系统热键的修饰按键
	//		其为 Alt(=1) ctrl(=2) shift(=4) Win(=8)的组合
	// bShowMsg 为是否再定义成功后显示热键的内容，默认是提示
	// 后来增加功能，如果用户想不改变 id 而改变全局热键，可以自己给出热键 id
	int SysHotkeySet(int keyCode, int shift=0,bool bShowMsg=false, int idHotkey = -1 );
	
	// 删除系统热键
	// 第一个参数为要移除的热键的 id
	// 第二个参数表示是否移除所有热键，如果为真自动忽略第一个参数
	bool SysHotkeyRemove(int idHotkey = -1, bool fRemoveAll = false);
...
private:
	...
	CBToolBar m_ToolBar;			// 用于访问和控制窗体上的一个工具栏，可以动态绑定到不同工具栏
	...
	//定义用于存储热键信息的 bool 数组大小
	// 感觉也不会太多，就定义 MaxSysHotKey 个吧，
	// 对于热键回收就查看标志变量，如果有真就用，没有就返回失败
	bool m_fHotkeyState[MaxSysHotKey];
...
//产生一个键盘消息
//idKey 表示键盘的键码
//iType 表示按键消息类型：0==按下抬起 -1==抬起 1==按下
//iDiffTime表示按键进行操作后的等待时间间隔，通常太快可能导致按键无效
void CreateKeybdEvent(int idKey, int iType = 0,int iDiffTime = 10);

//将键码转换为字符串输出返回，感觉按键提示需要用到这个比较多
